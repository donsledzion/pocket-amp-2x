...
namespace SoftAware.PocketAmp
{
    public class AndroidPlaybackEngine : IPlaybackEngine
    {
        ...
        private const int SettleTimeMs = 1000;
        ...
        private void UpdateNativeEQ(bool forced)
        {
            ...
            try
            {
                ...
                if (eqEnabled && lastBands != null)
                {
                    ...
                    var setBandLevelMethod = AndroidJNI.GetMethodID(currentClazz, "setBandLevel", "(SS)V");
                    if (setBandLevelMethod != IntPtr.Zero)
                    {
                        ...
                        for (short i = 0; i < cachedNumBands; i++)
                        {
                            var interpolatedGain = GetInterpolatedPocketAmpGain(cachedCenterFreqs[i]);
                            var finalGain = interpolatedGain + lastPreamp;
                            var clampedGain = Mathf.Clamp(finalGain, -20f, 20f);
                            var level = (short)Mathf.Lerp(cachedMinLevel, cachedMaxLevel, (clampedGain + 20f) / 40f);
                            ...
                            if (AndroidJNI.ExceptionOccurred() == IntPtr.Zero) continue;
        ...
        public void Update()
        {
            var currentTimeMs = _throttleClock.ElapsedMilliseconds;
            ...
            if (currentTimeMs - lastVolInteractionTimeMs < VolSettleTimeMs) return;
            isVolumeDirty = false;
            ApplyFinalVolume();
        }

        private float GetInterpolatedPocketAmpGain(float freqHz)
        {
            if (lastBands == null || lastBands.Length == 0) return 0f;
            
            float[] freqs = { 60f, 170f, 310f, 600f, 1000f, 3000f, 6000f, 12000f, 14000f, 16000f };
            
            if (freqHz <= freqs[0]) return lastBands[0];
            if (freqHz >= freqs[freqs.Length - 1]) return lastBands[freqs.Length - 1];
            
            // Find the two bands this frequency falls between
            for (var i = 0; i < freqs.Length - 1; i++)
            {
                if (!(freqHz >= freqs[i]) || !(freqHz <= freqs[i + 1])) continue;
                var t = (freqHz - freqs[i]) / (freqs[i + 1] - freqs[i]);
