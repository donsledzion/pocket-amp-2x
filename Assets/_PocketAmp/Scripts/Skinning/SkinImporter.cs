...
{
    /// <summary>
    /// Facade class that coordinates the loading of skins.
    ...
    {
        public static SkinImporter Instance { get; private set; }
        ...
        /// </summary>
        public async Task<Skin> LoadSkinAsync(string skinName)
        {
             Skin skin = new Skin { SkinName = skinName };
             string skinDir = fileSystem.GetSkinDirectory(skinName);

             if (!System.IO.Directory.Exists(skinDir))
             {
                 Debug.LogError($"[SkinImporter] Skin directory not found: {skinDir}");
                 return null;
             }
        ...
        private async Task LoadFullSkinAssets(Skin skin, string skinDir)
        {
             // 1. Main
        ...
        private async Task LoadSkinData(Skin skin, string skinDir)
        {
             // VISCOLOR
